...
import { getProviders, patientLookup, submitRequest } from "../services/api"
...
export default function Dashboard() {
  ...
  const handlePatientIdChange = (value) => {
    ...
    lookupTimer.current = setTimeout(async () => {
      ...
      try {
        const res = await patientLookup(encodeURIComponent(value))
        const p = res.data
        setFormData((prev) => ({
          ...prev,
  ...
  const handleNext = () => {
    setError("")
    if (!canProceed()) { setError("Please fill all required fields"); return }
    if (step === 0) { setStep(1); return }
    if (step === 1) { setStep(2); return }
    if (step === 2) {
      if (!showSimilarityCheck) { setShowSimilarityCheck(true); return }
      if (files.length < 3) { setError("Minimum 3 documents required"); return }
      setStep(3); return
    }
  }

  const handleSimilarityConfirm = () => { setError(""); setSimilarityConfirmed(true); setShowSimilarityCheck(false); setStep(3) }
  const handleSimilarityCancel = () => { setError(""); setShowSimilarityCheck(false) }

  const handleSubmit = async () => {
    setError("")
    if (files.length < 3) { setError("Minimum 3 healthcare documents required"); return }
  ...
  return (
    <PageTransition>
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-cyan-50/30 to-sky-50/30 dark:from-slate-950 dark:via-cyan-950/20 dark:to-sky-950/20 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-96 h-96 bg-cyan-300/10 dark:opacity-40 rounded-full blur-3xl" />
        <div className="absolute bottom-0 right-0 w-96 h-96 bg-sky-300/10 dark:opacity-40 rounded-full blur-3xl" />
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 py-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <button onClick={() => navigate("/doctor-dashboard")}
              className="flex items-center gap-2 bg-white/80 backdrop-blur-xl border border-slate-200 px-4 py-2.5 rounded-xl shadow-sm hover:shadow-md transition-all font-semibold text-slate-700 dark:text-slate-300 text-sm dark:bg-slate-800/80 dark:border-slate-700">
              <ArrowLeft size={18} /> Back
            </button>
            <div className="flex items-center gap-2 bg-cyan-50 border border-cyan-100 px-4 py-2 rounded-full dark:bg-cyan-950/40 dark:border-cyan-900/60">
              <Stethoscope size={14} className="text-cyan-600 dark:text-cyan-400" />
              <span className="text-xs font-semibold text-cyan-800 dark:text-cyan-300">New Prior Authorization</span>
            </div>
          </div>

          {/* STEP PROGRESS */}
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/30 shadow-lg p-4 mb-6 dark:bg-slate-900/80 dark:border-slate-700">
                  ...
                  <div key={s.id} className="flex items-center gap-2 flex-1">
                    <div className="flex items-center gap-2">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${
                        isDone ? "bg-emerald-500 text-white" : isActive ? "bg-gradient-to-r from-cyan-700 to-sky-600 text-white shadow-md" : "bg-slate-100 text-slate-400 dark:text-slate-500 dark:bg-slate-800"
                      }`}>
                        {isDone ? <CheckCircle2 size={18} /> : <Icon size={16} />}
                      </div>
                      <span className={`text-xs font-semibold hidden sm:block ${isActive ? "text-slate-900 dark:text-white" : "text-slate-400 dark:text-slate-500"}`}>{s.label}</span>
                    </div>
                    {i < STEPS.length - 1 && <div className={`flex-1 h-0.5 mx-2 rounded ${isDone ? "bg-emerald-400" : "bg-slate-200 dark:bg-slate-700"}`} />}
                  </div>
                )
              })}
            </div>
          ...
          {/* STEP CONTENT */}
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/30 shadow-lg p-6 min-h-[400px] flex flex-col dark:bg-slate-900/80 dark:border-slate-700">

            {/* INLINE ERROR */}
            {error && (
              <div className="mb-4 bg-red-50 border border-red-200 rounded-xl p-3 text-red-700 dark:bg-red-950/50 dark:border-red-900/60 dark:text-red-200 text-sm">
                {error}
              </div>
            )}
            ...
            {step === 0 && (
              <div className="flex-1 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5 dark:text-slate-400">Patient Name *</label>
                    <input type="text" placeholder="Full name" value={formData.patientName} onChange={(e) => setFormData({ ...formData, patientName: e.target.value })}
                      className="w-full border border-slate-200 rounded-xl py-2.5 px-4 bg-white/70 focus:outline-none focus:ring-2 focus:ring-cyan-400 text-sm text-slate-900 dark:text-white dark:bg-slate-800/70 dark:border-slate-700 dark:placeholder-slate-500" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5 dark:text-slate-400">Patient ID / Insurance ID *</label>
                    <div className="relative">
                      <input type="text" placeholder="Enter insurance ID or email" value={formData.patientId} onChange={(e) => handlePatientIdChange(e.target.value)}
                        className="w-full border border-slate-200 rounded-xl py-2.5 pl-10 pr-4 bg-white/70 focus:outline-none focus:ring-2 focus:ring-cyan-400 text-sm text-slate-900 dark:text-white dark:bg-slate-800/70 dark:border-slate-700 dark:placeholder-slate-500" />
                      <Search size={16} className="absolute left-3 top-3.5 text-slate-400 dark:text-slate-500" />
                      {lookupLoading && <Loader2 size={16} className="absolute right-3 top-3.5 text-cyan-500 animate-spin" />}
                    </div>
                    {lookupError && <p className="text-xs text-red-500 dark:text-red-400 mt-1">{lookupError}</p>}
                    {formData.patientName && !lookupLoading && (
                      <p className="text-xs text-emerald-600 dark:text-emerald-400 mt-1 font-medium">Patient: {formData.patientName}</p>
                    )}
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5 dark:text-slate-400">Diagnosis *</label>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5 dark:text-slate-400">Procedure Code</label>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5 dark:text-slate-400">Doctor Name</label>
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5 dark:text-slate-400">Insurance Provider *</label>
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-xs font-semibold text-slate-600 mb-1.5 dark:text-slate-400">Insurance ID *</label>
            ...
            {step === 1 && (
              <div className="flex-1 space-y-6">
                ...
                <div>
                  <label className="block text-xs font-semibold text-slate-600 mb-2 dark:text-slate-400">Urgency Level</label>
              ...
              <div className="flex-1 space-y-4">
                {!showSimilarityCheck ? (
                  <>
                    ...
                    <p className="text-xs text-slate-500 dark:text-slate-400">Upload at least <strong>3 healthcare documents</strong> (PDF, images) for AI analysis.</p>
                    <FileUpload setFiles={setFiles} />
                    {files.length > 0 && (
                      <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 flex items-center gap-2 dark:bg-emerald-950/40 dark:border-emerald-900/60">
                        <CheckCircle2 size={16} className="text-emerald-600 dark:text-emerald-400" />
                        <span className="text-sm text-emerald-700 font-medium dark:text-emerald-300">{files.length} file{files.length !== 1 ? "s" : ""} uploaded</span>
                      </div>
                    )}
                  </>
                ) : (
              ...
              <div className="flex-1 space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {[
                    ...
                    <div key={f.label} className="bg-slate-50 rounded-xl px-4 py-3 dark:bg-slate-800/80">
                      <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{f.label}</p>
                      <p className="text-sm font-semibold text-slate-900 dark:text-white mt-0.5">{f.value}</p>
                    </div>
                  ))}
                </div>
                <div className="bg-cyan-50 border border-cyan-100 rounded-xl p-4 flex items-start gap-3 dark:bg-cyan-950/40 dark:border-cyan-900/60">
                  <BadgeCheck size={20} className="text-cyan-600 dark:text-cyan-400 mt-0.5" />
                  <p className="text-sm text-cyan-800 dark:text-cyan-300">By submitting, you authorize the AI engine to process this request using the provided clinical documents and patient data.</p>
                </div>
              </div>
            )}
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-100 dark:border-slate-700">
              <button onClick={() => step > 0 ? setStep(step - 1) : navigate("/doctor-dashboard")}
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-700 font-semibold text-sm hover:bg-slate-50 transition-all dark:bg-slate-800 dark:border-slate-700 dark:text-white dark:hover:bg-slate-700">
