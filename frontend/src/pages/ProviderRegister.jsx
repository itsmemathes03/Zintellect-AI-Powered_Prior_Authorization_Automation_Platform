...
import { providerUnifiedRegister } from "../services/api"
...
export default function ProviderRegister() {
  ...
  const handleRegister = async () => {
    ...
    setError("")
    try {
      const res = await providerUnifiedRegister(formData)
      const data = res.data
      ...
      setError(err.response?.data?.detail || err.response?.data?.message || "Provider registration failed")
    }
    setLoading(false)
  }
          ...
          <div className="hidden lg:flex flex-col justify-between bg-white/80 dark:bg-slate-900/70 backdrop-blur-2xl border border-emerald-100/50 dark:border-emerald-900/50 rounded-[40px] p-10 xl:p-14 shadow-2xl min-h-[760px] relative overflow-hidden">
            ...
            <div className="text-center mt-10">
              <h1 className="text-4xl xl:text-5xl font-extrabold text-emerald-950 dark:text-white leading-tight">
                Join as an
                <span className="block text-emerald-600 dark:text-emerald-400 mt-2">Insurance Provider</span>
              </h1>
            ...
            <div className="space-y-5 mt-14">
              <div className="flex items-center gap-5 bg-white/70 dark:bg-slate-800/60 backdrop-blur-lg p-5 rounded-2xl shadow-md hover:scale-[1.02] transition-all duration-300">
                ...
                <div>
                  <h3 className="font-bold text-emerald-950 dark:text-white">AI-Powered Automation</h3>
                  <p className="text-slate-500 dark:text-slate-400 text-sm">Reduce authorization review time</p>
                </div>
              </div>
              <div className="flex items-center gap-5 bg-white/70 dark:bg-slate-800/60 backdrop-blur-lg p-5 rounded-2xl shadow-md hover:scale-[1.02] transition-all duration-300">
                ...
                <div>
                  <h3 className="font-bold text-emerald-950 dark:text-white">HIPAA Secure</h3>
              ...
              <div className="flex items-center gap-5 bg-white/70 dark:bg-slate-800/60 backdrop-blur-lg p-5 rounded-2xl shadow-md hover:scale-[1.02] transition-all duration-300">
                ...
                <div>
                  <h3 className="font-bold text-emerald-950 dark:text-white">Real-time Analytics</h3>
                  <p className="text-slate-500 dark:text-slate-400 text-sm">Monitor workflows instantly</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-emerald-100/40 dark:border-emerald-900/40 shadow-2xl rounded-[35px] p-6 sm:p-8 md:p-12 lg:p-14 relative overflow-hidden">
            ...
            <div className="text-center mb-10">
              <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold text-emerald-950 dark:text-white leading-tight">
                Provider <span className="text-emerald-600 dark:text-emerald-400">Registration</span>
              </h1>
            ...
            <div className="mb-6">
              <label className="block font-bold text-emerald-950 dark:text-emerald-100 mb-3 text-sm md:text-base">Insurance Provider Name</label>
