...
export default function PatientLayout() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-rose-50/20 to-pink-50/20 flex relative">
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 w-72 bg-white/80 backdrop-blur-xl border-r border-rose-100 flex flex-col transition-all duration-300 ${
        sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
      }`}>
        <div className="p-6 border-b border-rose-100">
          <Link to="/patient-dashboard" className="flex items-center gap-3 group">
            <div className="w-11 h-11 rounded-2xl bg-gradient-to-br from-rose-600 to-pink-600 flex items-center justify-center shadow-lg shadow-rose-200 group-hover:scale-110 transition-transform">
              <Heart className="text-white" size={22} />
            </div>
            <div>
              <h1 className="text-lg font-extrabold bg-gradient-to-r from-rose-700 to-pink-600 bg-clip-text text-transparent">Zintellect AI</h1>
              <p className="text-[10px] text-rose-500 tracking-widest uppercase font-semibold">Patient Portal</p>
            </div>
          </Link>
        </div>
            ...
            return (
              <Link
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-2xl font-semibold text-sm transition-all duration-200 group ${
                  isActive(link.path)
                    ? "bg-gradient-to-r from-rose-600 to-pink-600 text-white shadow-lg shadow-rose-200 scale-[1.02]"
      ...
      <div className="flex-1 flex flex-col min-h-screen relative z-10">
        <header className="sticky top-0 z-30 bg-white/70 backdrop-blur-xl border-b border-rose-100">
          <div className="flex items-center justify-between px-4 lg:px-6 h-16">
            <button
              onClick={() => setSidebarOpen(true)}
              className="lg:hidden p-2.5 rounded-2xl hover:bg-rose-50 transition-colors"
            >
            ...
            <div className="hidden lg:flex items-center gap-3">
              ...
              <span className="text-sm font-bold bg-gradient-to-r from-rose-700 to-pink-600 bg-clip-text text-transparent">{patientName}</span>
            </div>

            <div className="flex items-center gap-2">
              <button className="relative p-2.5 rounded-2xl hover:bg-rose-50 transition-colors group">
