...
import { submitSimilarityCheck } from "../services/api"
...
export default function DocumentSimilarityCheck({ files, onConfirm, onCancel }) {
  ...
  const runCheck = async () => {
    ...
    try {
      ...
      const data = await submitSimilarityCheck(formData)
  ...
  return (
    <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-3xl shadow-xl border border-slate-200 dark:border-slate-700 p-8">
      <div className="flex items-center gap-4 mb-6">
        <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-blue-900/40 dark:to-indigo-900/40 flex items-center justify-center shadow-inner">
          <FileSearch className="text-blue-700 dark:text-blue-400" size={24} />
        </div>
        <div>
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white">Document Similarity Check</h2>
          <p className="text-slate-500 dark:text-slate-400 text-sm">Detect duplicate or similar prior authorization submissions</p>
        </div>
      </div>
      ...
      {!result && !checking && (
        <div className="text-center py-8">
          ...
          <h3 className="text-xl font-bold text-slate-900 dark:text-white mt-6">Check for Duplicates</h3>
      ...
      {checking && (
        <div className="text-center py-12">
          ...
          <p className="text-slate-700 dark:text-slate-200 font-bold text-lg">Analyzing documents...</p>
          <p className="text-slate-500 dark:text-slate-400 mt-2">Comparing against existing submissions</p>
        </div>
      )}
        ...
        <div className={`rounded-2xl border p-6 ${getSeverityBg(result.max_similarity)}`}>
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-4 mt-4">
                <div className="bg-white/80 dark:bg-slate-800 rounded-xl px-4 py-3">
                  <p className="text-xs text-slate-500 dark:text-slate-400">Max Similarity</p>
                  <p className={`text-2xl font-extrabold ${getSeverityColor(result.max_similarity)}`}>
                    {(result.max_similarity * 100).toFixed(0)}%
                  </p>
                </div>
                <div className="bg-white/80 dark:bg-slate-800 rounded-xl px-4 py-3">
                  <p className="text-xs text-slate-500 dark:text-slate-400">Matches Found</p>
                  <p className="text-2xl font-extrabold text-slate-900 dark:text-white">{result.total_matches}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          {result.matches?.length > 0 && (
            <div className="mt-6 space-y-3">
              <h4 className="font-bold text-slate-900 dark:text-white">Matched Submissions</h4>
              {result.matches.map((match, idx) => (
                <div key={idx} className="bg-white/80 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 p-4 flex items-center justify-between">
                  <div>
                    <p className="font-semibold text-slate-900 dark:text-white">{match.patient_name || "Unknown Patient"}</p>
          ...
          <div className="flex items-center gap-4 mt-8">
            {result.is_suspected_duplicate ? (
              <>
                ...
                <button
                  onClick={onCancel}
                  className="flex-1 border border-slate-300 dark:border-slate-700 text-slate-700 dark:text-white py-4 rounded-2xl font-bold hover:bg-slate-50 dark:hover:bg-slate-800 transition-all flex items-center justify-center gap-2"
                >
                  <X size={20} /> Cancel
                </button>
              </>
      ...
      {confirmed && result && (
        <div className="mt-4 bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-900/60 rounded-2xl p-4 flex items-center gap-3">
          <CheckCircle2 className="text-emerald-600 dark:text-emerald-400" size={22} />
          <p className="text-emerald-700 dark:text-emerald-200 font-medium">Similarity check completed and acknowledged</p>
        </div>
      )}
    </div>
  )
