...
def patient_login(request: PatientLoginRequest):
    ...
    try:
        ...
        patient_name = user.first_name or (member.patient_name if member else "Patient")

        # Same JWT shape as /patient/login: sub = patient user id, email,
        # role = patient, name. Used by require_role("patient") endpoints
        # (profile, requests, stats, notifications) and /submit-request.
