...
def doctor_register(request: DoctorRegisterRequest, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        if not existing_doctor:
            # Identity is canonical in `users`; the doctor table stores
            # profile linkage only (no duplicate password).
            doctor = Doctor(
                id=str(uuid.uuid4()),
