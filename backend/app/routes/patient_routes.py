...
def patient_register(request: PatientRegisterRequest):
    ...
    try:
        ...
        try:
            ...
            if not email_ok:
                print(
                    f"[patient] Welcome email could not be sent "
                    f"(SMTP not configured or failed) for {request.email}"
                )
        except Exception as email_err:
            print(f"[patient] Welcome email error: {email_err}")

        # -------------------------------------------
        # DISPATCH PATIENT REGISTERED EVENT (fire-and-forget)
        # -------------------------------------------
