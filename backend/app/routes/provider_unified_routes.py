...
from app.models.enums import RequestDecision
...
def provider_register(request: ProviderRegisterRequest):
    ...
    try:
        ...
        if existing_user or existing_prov:
            raise HTTPException(
                status_code=409,
        ...
        # ==========================================
        # ATOMIC CREATION: shared ID for both tables
        # ==========================================
...
def update_request_status(
    ...
    # HCI-03: Requests in "Awaiting Review" must go through the HumanReview
    # workflow (POST /review/{request_id}/decide).  The old endpoint cannot
    # bypass the human-in-the-loop review process.
    if req.status == "Awaiting Review":
        raise HTTPException(
            status_code=409,
            detail=(
                f"Request {request_id} is in Awaiting Review status. "
                f"Use POST /review/{request_id}/decide for human review."
            ),
        )
    ...
    # Template-based PA status email (queued via BackgroundTasks)
    if patient_email:
        send_pa_status_email(
            background_tasks,
            patient_email,
            PAStatusUpdateContext(
                request_id=request_id,
                patient_name=req.patient_name,
                procedure_name=req.procedure_code or "",
